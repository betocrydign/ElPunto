 ...
 * -------------------------------------------------------------------------*/	
	...
	$my_meta->addSelect(THEME_NAME.'_sidebar_position_2',array('right'=>'Right','left'=>'Left'),array('name'=> __('Second Sidebar Position ','tax-meta'), 'std'=> array('right')));
	$my_meta->addSelect(THEME_NAME.'_sidebar_select_2', $sidebars ,array('name'=> __('Second Sidebar','tax-meta'), 'std'=> array('off')));
	$my_meta->addSelect(THEME_NAME.'_breaking_slider',array('show'=>'Show','slider_off'=>'Hide'),array('name'=> __('Breaking News Slider','tax-meta'), 'std'=> array('slider_off')));
...
class OT_Walker extends Walker_Nav_Menu {
    ...
    function start_el(&$output, $item, $depth = 0, $args = array(), $current_object_id = 0) {
			...
			if($depth==2 && $count==1) {
		        ...
		        if($item->object=="category"){
					...
					$the_query = new WP_Query($postArgs);
						$item_output .= '<div class="article-block">';
							if ($the_query->have_posts()) : while ($the_query->have_posts()) : $the_query->the_post();
								...
								//get post meta info
								$image = get_post_thumb($the_query->post->ID,$width,$height); 
								//post details
								...
								$item_output .= '<div class="item'.$itemClass.'">';
									$item_output .='<div class="item-header">';
										$item_output .='<a href="'.get_permalink().'" class="image-hover"><img src="'.$image['src'].'" alt="'.esc_attr(get_the_title()).'" title="'.esc_attr(get_the_title()).'" /></a>';
									$item_output .='</div>';

									$item_output .='<div class="item-content">';
										$item_output .='<h4><a href="'.get_permalink().'">'.get_the_title().'</a></h4>';
										add_filter('excerpt_length', 'new_excerpt_length_10');	
										$item_output .='<p>'.get_the_excerpt().'</p>';
										remove_filter('excerpt_length', 'new_excerpt_length_10');	

	
										if(comments_open() && ot_option_compare($postDate,$postDateSingle)==true) { 
											$item_output .='<span>'.get_the_time(get_option('date_format')).'</span>';
										}
									$item_output .='</div>';
								$item_output .='</div>';
								$i++;
					...
					$item_output .='<div class="article-block"><p>'.__("Please Insert A category", THEME_NAME).'</p></div>';
				}

			} elseif($depth==2 && $count==3) {
		        if($item->object=="category"){
		        	$postArgs=array(
						'cat'=> $item->object_id,
						'posts_per_page'=> 3,
					...
					$the_query = new WP_Query($postArgs);
						$item_output .= '<div class="article-block">';
							if ($the_query->have_posts()) : while ($the_query->have_posts()) : $the_query->the_post();
								//get post meta info
								$image = get_post_thumb($the_query->post->ID,51,51); 
								//post details
								$postDate = get_option(THEME_NAME."_post_date_single");
								$postDateSingle = get_post_meta ( $the_query->post->ID, "_".THEME_NAME."_post_date", true ); 
		...
		} else if(($parent_menu_type=="3") && ($depth==0)) {
	        $attributes  = ! empty( $item->attr_title ) ? ' title="'  . esc_attr( $item->attr_title ) .'"' : '';
	        $attributes .= ! empty( $item->target )     ? ' target="' . esc_attr( $item->target     ) .'"' : '';
	        $attributes .= ! empty( $item->xfn )        ? ' rel="'    . esc_attr( $item->xfn        ) .'"' : '';
	        $attributes .= ! empty( $item->url )        ? ' href="'   . esc_attr( $item->url        ) .'"' : '';
 ...
 * -------------------------------------------------------------------------*/
	...
	function ot_get_comments_number($id) {
		...
		if ( $num_comments == 0 ) {
			$comments = __('<strong>0</strong> comments', THEME_NAME);
		} elseif ( $num_comments > 1 ) {
			$comments = '<strong>'.$num_comments.'</strong>' . __(' comments', THEME_NAME);
		} else {
			$comments = __('<strong>1</strong> comment', THEME_NAME);
		}
		return $comments;
	}
...
$orangethemes_buttons=array("orangethemesbutton", "orangethemesspacer", "orangethemesquote", "|",
			 "orangethemeslists", "|","orangethemesvideo","orangethemesgallery" ,"orangethemesmarker","orangethemeslink","orangethemestabs", "|",
   ...
   ?>
	...
	<li <?php comment_class($comentClass); ?> id="li-comment-<?php comment_ID(); ?>">
		<div class="comment-block" id="comment-<?php comment_ID() ?>">
			<?php if(ot_get_avatar_url(get_avatar( $comment, 80))) { ?>
				<a href="<?php if(get_comment_author_url()) { echo get_comment_author_url();} else { echo "#"; } ?>" class="image-avatar">
					<img src="<?php echo ot_get_avatar_url(get_avatar( $comment, 80));?>" alt="<?php echo get_comment_author();?>" title="<?php echo get_comment_author();?>" />
				</a>
			<?php } ?>
				<div class="comment-text">
					<?php if($depth==1) { ?>
						<a href="#comment-<?php comment_ID() ?>" class="reply-id">#<?php echo $orangeThemesCommentID;?></a>
					<?php } ?>
					...
					<span class="time-stamp"><?php printf(__(' %1$s, %2$s', THEME_NAME), get_comment_date("j F, Y"), get_comment_time("H:i"));?></span>
					<?php comment_text(); ?>
